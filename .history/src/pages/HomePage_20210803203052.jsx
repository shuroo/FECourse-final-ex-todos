...
function HomePage(){
   ...
   function checkIfDone(taskId){
    ...
    useEffect(() => {
        ...
        const compeletedTasks = allTasks.filter((task)=>{
            console.log(task);
            return (!checkIfDone(task));
        });
        const activeTasks = allTasks.filter((task)=>checkIfDone(task));
        setTasks(activeTasks);
    }, [filterBy]) 
    

    const tasksView = (!tasks ? null : tasks.filter((task)=>!task.isDone).map((task,index) =>{ 
